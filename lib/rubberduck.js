	...
	fn : function(emitter, fn, name, scope) {
		...
		return function() {
			...
			try {
				...
				ucName && emitter.emit('after' + ucName, e, arguments, this, name);
				emitter.emit('after', e, arguments, this, name);
				throw e;
			}
		...
		return function() {
			...
			callbackWrapper = function() {
				try {
					...
					ucName && emitter.emit('after' + ucName, e, methodArgs, context, name);
					emitter.emit('after', e, methodArgs, context, name);
					throw e;
				}
			...
			try {
				...
				ucName && emitter.emit('after' + ucName, e, arguments, context, name);
				emitter.emit('after', e, arguments, context, name);
				throw e;
			}
		}
	}
